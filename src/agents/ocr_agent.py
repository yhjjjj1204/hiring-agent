"""Resume OCR and structured parsing Agent based on OpenAI GPT-4o."""
...
def extract_resume_text_from_path(file_path: str | Path) -> str:
    """Read image/PDF resume and extract Markdown text."""
    path = Path(file_path)
...
def extract_and_arrange_resume_from_path(
    ...
    """Complete OCR and structured resume parsing in a single call."""
    path = Path(file_path)

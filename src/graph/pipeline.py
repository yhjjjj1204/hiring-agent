"""Stateful hiring main workflow: HR requirements -> OCR -> Data arrangement -> Background analysis -> Auto-scoring + HITL."""
...
class HiringPipelineState(TypedDict, total=False):
    """Main LangGraph state; nodes write to corresponding keys. HITL merge logic for values via interrupts."""
...
def extract_hr_job_spec_from_text(hr_requirement_text: str) -> dict[str, Any]:
    """Extract structured job spec from free-form HR text (reusable for sync reviews, skips HITL)."""
...
def _name_background_mismatch(arr: dict[str, Any], bg: dict[str, Any]) -> str | None:
    ...
    if cn.replace(" ", "").lower() == dn.replace(" ", "").lower():
        return None
    # Simple Latin/Chinese mix, no full NLP; just alert the user.
...
def node_background(state: HiringPipelineState) -> dict[str, Any]:
    ...
    if mismatch:
        # In dev/batch scenarios, do not block the pipeline if academic search results in homonym mismatch;
        # Discard academic branch and proceed with scoring (scoring uses blinded snapshot).
...
def node_fairness_blinding(state: HiringPipelineState) -> dict[str, Any]:
    """Blinded screening: Generate a redacted snapshot for scoring (full resume remains in arranged_resume)."""
    arr = state.get("arranged_resume") or {}
...
def build_hiring_pipeline_graph():
    """Compile the main workflow with checkpointer (In-memory; replace with PostgresSaver for production)."""
